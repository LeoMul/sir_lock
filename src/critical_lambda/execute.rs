...
pub fn run_simulation(param:CriticalLambdaParams, json: Value, num_threads: Option<NonZeroUsize>){
    match param.graph_type{
        GraphType::SmallWorld(_) => sim_small_world_in_boots_single_energy(param, json, num_threads),
...
fn sim_small_world_in_boots_single_energy(param: CriticalLambdaParams,json:Value,num_threads:Option<NonZeroUsize>){
    ...
    for n in n_size{
        ...
        if param.bootbool{
            ...
            let end = merged_vec.len()/k.get();
            let rngs: Vec<_> = (0..k.get()).map(
                |_|
                {
            ...
            let measure_data:Vec<MyVarianceInBoots> =iterator.progress_with(bar).map(
                |(container,slice)|{
            ...
            writing_with_boot_single_energy(param.clone(), json.clone(), num_threads, &measure_data, n, lambda_vec.clone())
            }
        else{
            //do not resample and produce data file.print
            //println!("Skipping bootstrapping.")
            ...
            writing_with_no_boot_single_energy(param.clone(), json.clone(), num_threads, &measure_data, n, lambda_vec.clone())
        }
            ...
            |(r,graph_rng)|
            {
                ...
                iter.map(
                    ...
                    let data_point_for_each_lambda:Vec<_> = lambda_vec.iter().map(|lambda|{
                        model.set_lambda(*lambda);
                        model.reseed_sir_rng(r);
                        let m = model.propagate_until_completion_max_with_lockdown(&mut lockgraph,lockparams) as f64;
