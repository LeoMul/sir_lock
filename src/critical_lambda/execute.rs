...
pub fn run_simulation(param:CriticalLambdaParams, json: Value, num_threads: Option<NonZeroUsize>){
    match param.graph_type{
        GraphType::SmallWorld(_) => sim_small_world_again(param, json, num_threads),
        _ => unimplemented!()
    }
...
fn sim_small_world_again_rewriting(param:CriticalLambdaParams, json: Value, num_threads: Option<NonZeroUsize>){
    ...
    let data_vec:Vec<_> = n_size.iter_mut().map(|n|{
        ...
        let n_lambdaspanned:Vec<_> = lambda_vec.iter().map(|lambda|{
            ...
            let k = num_threads.unwrap().get();

            let per_thread = param.num_networks as usize /k;

            let mut container: Vec<_> = (0..k).map(|_|{}).collect();

            let thread_data :Vec<_> = container.par_iter_mut().map(|_|{
                let mut avg_c_p = 0.;
                let mut var_c_p = 0.;
                let mut rng = Pcg64::seed_from_u64(param.sir_seed);
            ...
            for (i,j) in thread_data{
                avg_c += i;
                var_c += j;
            }
            avg_c /= (k*per_thread) as f64;
            var_c /= (k*per_thread) as f64;
            println!("{}",avg_c);
            MyVariance{
                mean: avg_c,
                var: var_c
        ...
        let critical_lambda = find_critical_lambda_one_data_set(n_lambdaspanned, &lambda_vec);
        (*n,critical_lambda)
    ...
    alternate_writing(param, json, num_threads, data_vec);

}

fn sim_small_world_again(param:CriticalLambdaParams, json: Value, num_threads: Option<NonZeroUsize>){
...
fn alternate_writing(param:CriticalLambdaParams,json:Value,num_threads: Option<NonZeroUsize>,data:Vec<(usize,f64)>){
    let name = param.name("dat", num_threads);
        println!("creating: {name}");
        let file = File::create(name).expect("unable to create file");
        let mut buf = BufWriter::new(file);
        ...
        writeln!(buf).unwrap();
       // let data_vec = &data[n];
        writeln!(buf, "#n critical_lambda").unwrap();
        for j in 0..data.len(){
    
            writeln!(buf,"{} {}",data[j].0,data[j].1).unwrap();
        }
