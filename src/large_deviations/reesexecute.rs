...
pub fn execute_entropic_sampling(json_opts: &REESJsonOpts,start_time:Instant,rewltype:RewlType){
    
    match rewltype{
        RewlType::SmallWorld => execute_entropic_sampling_sw(json_opts, start_time),
        RewlType::Barabasi => execute_entropic_sampling_ba(json_opts, start_time),
        _ => {  println!("Invalid graph type specified in json");
                unimplemented!()}
    }
}


pub fn execute_entropic_sampling_ba(opt: &REESJsonOpts, start_time: Instant)
{       
    let print_count = opt.print_count.unwrap().get();
    if print_count == 0{
        println!("Zero print count, panic!");
        panic!()
    }
    let mut log_file = LogfilePrinter::new("rees.log");
    let allowed = opt.allowed_seconds();
    writeln!(log_file, "Opening {}", &opt.file_name).unwrap();

    // read in save state
    let (rewl, old_json_string) = deserialize_from_file(&opt.file_name);
    ...
    let writer: Vec<_> = rewl.hists()
        ...
        .map(
            |((index, hist), walker)|
            {
                ...
                let ever = walker.step_count() / print_count;
...
pub fn execute_entropic_sampling_sw(opt: &REESJsonOpts, start_time: Instant)
{
    let print_count = opt.print_count.unwrap().get();
    if print_count == 0{
        println!("Zero print count, panic!");
