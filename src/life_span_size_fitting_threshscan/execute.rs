...
fn sim_barabasi(param: LifespanSizeFittingThreshParams, json: Value, num_threads:Option<NonZeroUsize>){
    ...
    println!("{:?}",n_size);
    let thresh_range = param.lockdownthreshrange.get_range();
    ...
    for n in n_size{
        ...
        let intermediate_data_vecs: Vec<_> = rngs.par_iter_mut().map(|(rng,graph_rng)|{
            ...
            iter.map(|_|{
                ...
                let opt = BarabasiOptions::from_life_span_fiting_thresh_params(&param,NonZeroUsize::new(n).unwrap(),new_graph_seed);
       ...
       alternate_writing(&param, &json, num_threads, x, n, &thresh_vec);

    };

}

fn sim_small_world(param: LifespanSizeFittingThreshParams, json: Value, num_threads:Option<NonZeroUsize>){
    let k = num_threads.unwrap_or_else(|| NonZeroUsize::new(1).unwrap());
    rayon::ThreadPoolBuilder::new().num_threads(k.get()).build_global().unwrap();
...
fn alternate_writing(param:&LifespanSizeFittingThreshParams,json:&Value,num_threads: Option<NonZeroUsize>,data_master:Vec<u32>,n:usize,lambda:&[f64]){
