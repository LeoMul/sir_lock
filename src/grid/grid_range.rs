...
#[derive(Clone)]
pub struct GridRangeUsize{
    range: RangeInclusive<usize>,
    steps: NonZeroUsize
}
impl GridRangeUsize{
    pub fn new_native_usize(start:usize,end:usize,steps:NonZeroUsize) -> Self{
        Self{
    ...
    ) -> Self
    {
        ...
        Self::new_native_usize(start, end, steps_m1)
    }
    pub fn iter(&self) -> GridRangeIterUsize
    {
        GridRangeIterUsize{
...
impl Iterator for GridRangeIterUsize 
{
    type Item = usize;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let next = self.start + (((self.end - self.start) as f64) * (self.current as f64/ self.steps_m1.get() as f64)) as usize;
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item>
    {
        ...
        {
            let next = self.start + (self.end - self.start) * (self.current/self.steps_m1.get());
            self.exhausted = self.current == self.steps_m1.get();
            if !self.exhausted{
                self.current += 1;
            }
...
mod grid_range_tests
{
    ...
    use net_ensembles::rand::{SeedableRng, distributions::*};
